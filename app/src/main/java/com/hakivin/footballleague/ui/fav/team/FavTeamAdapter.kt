...
class FavTeamAdapter (val list: List<TeamItem>): RecyclerView.Adapter<FavTeamAdapter.ViewHolder>() {
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(team : TeamItem){
            Picasso.get().load(team.desc).into(itemView.team_list_image)
            itemView.team_list_name.text = team.name
            itemView.team_list_stadium.text = team.banner
